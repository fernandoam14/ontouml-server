...
import { Project, Ontouml2Alloy } from 'ontouml-js';
...
function performTransformation(project: Project, options: any): any {
  const service = new Ontouml2Alloy(project, options);
  return service.run();
}

function logTransformationRequest(request: express.Request): void {
  console.log(`------------------------------------`);
  console.log(`[${new Date().toISOString()}] - Processing transformation to relational database request`);
